...
void searchPKI(int zIndex, int pIndex, int argc, char** argv, Zipcode zArray[], PKIStruct pArray[]){
	...
	for(int i = 1; i < argc; i++){
		...
		if(!found){
			std::cout << "Zipcode " << arg << " was not found within this file!" << std::endl;
		}
...
{
	//std::cout<<(char)30;
	///(char)30 is called record seperator, and it is not rendered in CentOS
	...
	///Segmentation fault occurs when creating block array.
	Block BlockArray[15000];//should be enough so long as at least three zipcodes can fit in a typical block.
